...
def _get_rotation(e: Ellipsoid, coords: Vector) -> np.ndarray:
    up = Vector(0, 0, 1)
...
def _render_socket(name: str, coords: Vector, params: InputParameters,) -> Mesh.Mesh:
    point = Mesh.Mesh(params.socket.path)
    quat = _get_rotation(params.ellipsoid, coords)

    rotation = App.Rotation(quat[0], quat[1], quat[2], quat[3])
    vector = App.Vector(coords[0], coords[1], coords[2])
    point.Placement = App.Placement(vector, rotation)
